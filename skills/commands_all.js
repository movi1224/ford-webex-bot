...
var r = $.ajax(settings);
r.done(	function (response) {	result1 = checkResult(response, 1);});
...
r.done( function (response) {
      ...
      else
      {
        ...
        if (i != ops - 2 && i != ops - 1) { result2 += ", "; }
        else if (i != ops - 1) { result2 += ", & "; }
        else {}
      }
}});
...
var r = $.ajax(settings);
r.done( function (response) { result3 = checkResult(response, 1);});
...
r.done(function(response) {
  ...
  for (var i = 0; i < ops; i++) {
      ...
      else
      {
        ...
        if (i != ops - 2 && i != ops - 1) { result4 += ", "; }
        else if (i != ops - 1) { result4 += ", & "; }
        else {}
      }
}});
...
var r = $.ajax(settings);
r.done( function (response) { result5 = checkResult(response, 1);});
...
var r = $.ajax(settings);
r.done( function (response) { result6 = checkResult(response, 1);});
...
var r = $.ajax(settings);
r.done( function (response) { result7 = checkResult(response, 1);});
...
var r = $.ajax(settings);
r.done( function (response) { result8 = checkResult(response, 1);});
...
var r = $.ajax(settings);
r.done( function (response) { result9 = checkResult(response, 1);});
...
var r = $.ajax(settings);
r.done( function (response) { result10 = checkResult(response, 1);});
...
r.done(function(response) {
  ...
  for (var i = 0; i < ops; i++) {
      ...
      else
      {
        ...
        if (i != ops - 2 && i != ops - 1) { result11 += ", "; }
        else if (i != ops - 1) { result11 += ", & "; }
        else {}
      }
}});
...
var r = $.ajax(settings);
r.done( function (response) { result12 = checkResult(response, 1);});
...
var r = $.ajax(settings);
r.done( function (response) { result13 = checkResult(response, 1);});
...
var r = $.ajax(settings);
r.done( function (response) { result14 = checkResult(response, 1);});
...
var r = $.ajax(settings);
r.done( function (response) { result15 = checkResult(response, 1);});
...
var r = $.ajax(settings);
r.done( function (response) { result16 = checkResult(response, 1);});
...
var r = $.ajax(settings);
r.done( function (response) { result17 = checkResult(response, 1);});
...
r.done(function(response) {
  ...
  for (var i = 0; i < ops; i++) {
      ...
      else
      {
        ...
        if (i != ops - 2 && i != ops - 1) { result18 += ", "; }
        else if (i != ops - 1) { result18 += ", & "; }
        else {}
      }
}});
...
var r = $.ajax(settings);
r.done( function (response) { result19 = checkResult(response, 3, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result20 = checkResult(response, 4, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result21 = checkResult(response, 4, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result22 = checkResult(response, 4, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result23 = checkResult(response, 3, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result24 = checkResult(response, 4, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result25 = checkResult(response, 4, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result26 = checkResult(response, 4, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result27 = checkResult(response, 3, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result28 = checkResult(response, 4, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result29 = checkResult(response, 4, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result30 = checkResult(response, 4, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result31 = checkResult(response, 3, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result32 = checkResult(response, 4, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result33 = checkResult(response, 4, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result34 = checkResult(response, 4, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result35 = checkResult(response, 3, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result36 = checkResult(response, 4, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result37 = checkResult(response, 4, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result38 = checkResult(response, 4, true);});
...
var r = $.ajax(settings);
r.done( function (response) { result39 = formatProductUsage(response);});
...
var r = $.ajax(settings);
r.done( function (response) { result40 = formatProductUsage(response, 'trusted-consumer');});
...
settings.data =
    ...
    r.done(function(response) {
      ...
      result43 = "In the last 24 hours the total traffic of requests shifted from " +
          last24h +
          " to " +
          newrate +
          "."
...
// Goodbye
var goodbyes = ["see you", "bye", "goodbye", "farewell", "have a good", "take care", "see ya", "later", "peace"];
...
// Least called API
var least_called_api = ["least called API", "least called application programming interface", "What is my least called API?", "What is my least called application programming interface?"];
all_commands = all_commands.concat(least_called_api);

// 5 Least called APIs
var least_called_apis = ["least 5 called API", "least five called API", "least 5 called application programming interface", 
                         "least five called application programming interface", "5 least called api", "five least called api",
                        "What are my least 5 called APIs?","What are my least 5 called application programming interfaces?",
                        "What are my least five called APIs?","What are my least five called application programming interfaces?",
                        "What are my 5 least called APIs?","What are my 5 least called application programming interfaces?",
                        "What are my five least called APIs?","What are my five least called application programming interfaces?",
                        "5 least called API", "five least called API", "5 least called application programming interface", 
                         "five least five called application programming interface", "5 least called api", "five least called api",
                        "What are my 5 least called APIs?","What are my 5 least called application programming interfaces?"];
all_commands = all_commands.concat(least_called_apis);

// most called API
var most_called_api = ["most called API", "most called application programming interface", "What is my most called API?", "What is my most called application programming interface?",
                      "top called API", "top called application programming interface", "What is my top called API?", "What is my top called application programming interface?"];
all_commands = all_commands.concat(most_called_api);

// 5 most called APIs
var most_called_apis = ["most 5 called API", "most five called API", "most 5 called application programming interface", 
                         "most five called application programming interface", "top 5 API", "top 5 called API", 
                        "top 5 called application programming interface", "top 5 called called application programming interface",
                        "What are my top 5 called APIs?","What are my top 5 called application programming interfaces?",
                        "What are my top five called APIs?","What are my top five called application programming interfaces?",
                        "What are my 5 top called APIs?","What are my 5 top called application programming interfaces?",
                        "What are my five top called APIs?","What are my five top called application programming interfaces?",
                        "What are my most 5 called APIs?","What are my most 5 called application programming interfaces?",
                        "What are my most five called APIs?","What are my most five called application programming interfaces?",
                        "What are my 5 most called APIs?","What are my 5 most called application programming interfaces?",
                        "What are my five most called APIs?","What are my five most called application programming interfaces?"];
...
// Least called operation
var least_called_operation = ["least called operation", "What is my least called operation?"];
all_commands = all_commands.concat(least_called_operation);

// least called operation for 'sessions'
var least_called_operation_sessions = ["least called operation for sessions", "What is my least called operation for sessions?"];
all_commands = all_commands.concat(least_called_operation_sessions);

// least called operation for 'vehicles'
var least_called_operation_vehicles = ["least called operation for vehicles", "What is my least called operation for vehicles?"];
all_commands = all_commands.concat(least_called_operation_vehicles);

// least called operation for 'client-gateway-users'
var least_called_operation_cgu = ["least called operation for client-gateway-users", "What is my least called operation for client-gateway-users?"];
all_commands = all_commands.concat(least_called_operation_cgu);

// least called operation for 'trusted-consumer'
var least_called_operation_tc = ["least called operation for trusted-consumer", "What is my least called operation for trusted-consumer?"];
all_commands = all_commands.concat(least_called_operation_tc);

// least called operation for 'trusted-services'
var least_called_operation_ts = ["least called operation for trusted-services", "What is my least called operation for trusted-services?"];
all_commands = all_commands.concat(least_called_operation_ts);

// 5 Least called operations
var least_called_operations = ["least 5 called operation", "least five called operation", "What are my least 5 called operations?", "What are my least five called operations?",
                              "5 least called operation", "five least called operation", "What are my 5 least called operations?", "What are my five least called operations?"];
all_commands = all_commands.concat(least_called_operations);

// Most called operation
var most_called_operation = ["most called operation", "top called operation", "What is my most called operation?", "What is my top called operation?"];
all_commands = all_commands.concat(most_called_operation);

// Most called operation for 'sessions'
var most_called_operation_sessions = ["top called operation for sessions", "most called operation for sessions", "What is my top called operation for sessions?", "What is my most called operation for sessions?"];
all_commands = all_commands.concat(most_called_operation_sessions);

// Most called operation for 'vehicles'
var most_called_operation_vehicles = ["top called operation for vehicles", "most called operation for vehicles", "What is my top called operation for vehicles?", "What is my most called operation for vehicles?"];
all_commands = all_commands.concat(most_called_operation_vehicles);

// Most called operation for 'client-gateway-users'
var most_called_operation_cgu = ["top called operation for client-gateway-users", "most called operation for client-gateway-users", "What is my top called operation for client-gateway-users?", "What is my most called operation for client-gateway-users?"];
all_commands = all_commands.concat(most_called_operation_cgu);

// Most called operation for 'trusted-consumer'
var most_called_operation_tc = ["top called operation for trusted-consumer", "most called operation for trusted-consumer", "What is my top called operation for trusted-consumer?", "What is my most called operation for trusted-consumer?"];
all_commands = all_commands.concat(most_called_operation_tc);

// Most called operation for 'trusted-services'
var most_called_operation_ts = ["top called operation for trusted-services", "most called operation for trusted-services", "What is my top called operation for trusted-services?", "What is my most called operation for trusted-services?"];
all_commands = all_commands.concat(most_called_operation_ts);

// 5 Most called operations
var most_called_operations = ["5 most called operation", "five most called operation", "What are my top 5 called operations?", "What are my least five called operations?",
                              "What are my most 5 called operations?", "What are my most five called operations?", "5 top called operation", "five top called operation", 
...
// Average response time for the operation 'GetVehicle'
var avg_response_time_gv = ["average response time for GetVehicle", "What is the average response time for GetVehicle?"];
all_commands = all_commands.concat(avg_response_time_gv);

// Average response time for the operation 'GetVehicle' of the API, 'sessions'
var avg_response_time_gv_sessions = ["average response time for the operation GetVehicle of the API sessions", "What is the average response time for GetVehicle of the API sessions?"];
all_commands = all_commands.concat(avg_response_time_gv_sessions);

// Average response time for the operation 'GetVehicle' of the API, 'client-gateway-users'
var avg_response_time_gv_cgu = ["average response time for the operation GetVehicle of the API client-gateway-users", "What is the average response time for GetVehicle of the API client-gateway-users?"];
all_commands = all_commands.concat(avg_response_time_gv_cgu);

// Average response time for the operation 'GetVehicle' of the API, 'vehicles'
var avg_response_time_gv_v = ["average response time for the operation GetVehicle of the API vehicles", "What is the average response time for GetVehicle of the API vehicles?"];
all_commands = all_commands.concat(avg_response_time_gv_v);

// Average response time for the operation 'GetAuthorizationStatus'
var avg_response_time_gas = ["average response time for GetAuthorizationStatus", "What is the average response time for GetAuthorizationStatus?"];
all_commands = all_commands.concat(avg_response_time_gas);

// Average response time for the operation 'GetAuthorizationStatus' of the API, 'sessions'
var avg_response_time_gas_sessions = ["average response time for GetAuthorizationStatus of the API sessions", "What is the average response time for GetAuthorizationStatus of the API sessions?"];
all_commands = all_commands.concat(avg_response_time_gas_sessions);

// Average response time for the operation 'GetAuthorizationStatus' of the API, 'client-gateway-users'
var avg_response_time_gas_cgu = ["average response time for GetAuthorizationStatus of the API client-gateway-users", "What is the average response time for GetAuthorizationStatus of the API client-gateway-users?"];
all_commands = all_commands.concat(avg_response_time_gas_cgu);

// Average response time for the operation 'GetAuthorizationStatus' of the API, 'vehicles'
var avg_response_time_gas_v = ["average response time for GetAuthorizationStatus of the API vehicles", "What is the average response time for GetAuthorizationStatus of the API vehicles?"];
all_commands = all_commands.concat(avg_response_time_gas_v);

// Average response time for the operation 'GetUserProfile'
var avg_response_time_gup = ["average response time for GetUserProfile", "What is the average response time for GetUserProfile?"];
all_commands = all_commands.concat(avg_response_time_gup);

// Average response time for the operation 'GetUserProfile' of the API, 'sessions'
var avg_response_time_gup_sessions = ["average response time for GetUserProfile of the API sessions", "What is the average response time for GetUserProfile of the API sessions?"];
all_commands = all_commands.concat(avg_response_time_gup_sessions);

// Average response time for the operation 'GetUserProfile' of the API, 'client-gateway-users'
var avg_response_time_gup_cgu = ["average response time for tGetUserProfile of the API client-gateway-users", "What is the average response time for GetUserProfile of the API client-gateway-users?"];
all_commands = all_commands.concat(avg_response_time_gup_cgu);

// Average response time for the operation 'GetUserProfile' of the API, 'vehicles'
var avg_response_time_gup_v = ["average response time for GetUserProfile of the API vehicles", "What is the average response time for GetUserProfile of the API vehicles?"];
all_commands = all_commands.concat(avg_response_time_gup_v);

// Average response time for the operation 'GetSecurityQuestions'
var avg_response_time_gsq = ["average response time for GetSecurityQuestions", "What is the average response time for GetSecurityQuestions?"];
all_commands = all_commands.concat(avg_response_time_gsq);

// Average response time for the operation 'GetSecurityQuestions' of the API, 'sessions'
var avg_response_time_gsq_sessions = ["average response time for GetSecurityQuestions of the API sessions", "What is the average response time for GetSecurityQuestions of the API sessions?"];
all_commands = all_commands.concat(avg_response_time_gsq_sessions);

// Average response time for the operation 'GetSecurityQuestions' of the API, 'client-gateway-users'
var avg_response_time_gsq_cgu = ["average response time for GetSecurityQuestions of the API client-gateway-users", "What is the average response time for GetSecurityQuestions of the API client-gateway-users?"];
all_commands = all_commands.concat(avg_response_time_gsq_cgu);

// Average response time for the operation 'GetSecurityQuestions' of the API, 'vehicles'
var avg_response_time_gsq_v = ["average response time for GetSecurityQuestions of the API vehicles", "What is the average response time for GetSecurityQuestions of the API vehicles?"];
all_commands = all_commands.concat(avg_response_time_gsq_v);

// Average response time for the operation 'Login'
var avg_response_time_l = ["average response time for Login", "What is the average response time for Login?"];
all_commands = all_commands.concat(avg_response_time_l);

// Average response time for the operation 'Login' of the API, 'sessions'
var avg_response_time_l_sessions = ["average response time for Login of the API sessions", "What is the average response time for Login of the API sessions?"];
all_commands = all_commands.concat(avg_response_time_l_sessions);

// Average response time for the operation 'Login' of the API, 'client-gateway-users'
var avg_response_time_l_cgu = ["average response time for Login of the API client-gateway-users", "What is the average response time for Login of the API client-gateway-users?"];
all_commands = all_commands.concat(avg_response_time_l_cgu);

// Average response time for the operation 'Login' of the API, 'vehicles'
var avg_response_time_l_v = ["average response time for Login of the API vehicles", "What is the average response time for Login of the API vehicles?"];
...
// How is my product usage trending week over week [for {product_name}]
var product_usage = ["How is my product usage trending week over week?", "What is my product usage like week over week?",
                    "How is my product usage trending over the past month?", "What is my product usage like over the past month?"];
all_commands = all_commands.concat(product_usage);

// How is my product usage trending week over week for 'trusted-consumer'
var product_usage_tc = ["How is my product usage trending week over week for trusted-consumer?", "What is my product usage like week over week for trusted-consumer?",
                    "How is my product usage trending over the past month for trusted-consumer?", "What is my product usage like over the past month for trusted-consumer?",
                    "How is my product usage for trusted-consumer trending over the past month?", "What is my product usage for trusted-consumer like over the past month?"];
all_commands = all_commands.concat(product_usage_tc);

// How is my product usage trending week over week for 'trusted-services'
var product_usage_ts = ["How is my product usage trending week over week for trusted-services?", "What is my product usage like week over week for trusted-services?",
                    "How is my product usage trending over the past month for trusted-services?", "What is my product usage like over the past month for trusted-services?",
                    "How is my product usage for trusted-services trending over the past month?", "What is my product usage for trusted-services like over the past month?"];
  ...
  controller.hears('.*', 'direct_message,direct_mention', function(bot, message) {
    ...
    /////////////////////////
      ...
      else if (least_called_apis.includes(command_result[0][1]) == true) 
      {
        ...
        if (commas != 5)
        {  
          text += "I'm sorry. I was not able to find 5 APIs, but I was able to find the " + commas + " least called APIs. ";
        };
        text += "Your least " + commas + " called APIs are " + result2 + ".";
      }
      ...
      // Checks if user is asking for the 5 most called APIs
        ...
        if (commas != 5)
        {  
          text += "I'm sorry. I was not able to find 5 APIs, but I was able to find the " + commas + " most called APIs. ";
        };
    ...
    /////////////////////////
      ...
      else if (least_called_operation_sessions.includes(command_result[0][1]) == true) {
        var text = "Your least called operation for 'sessions' is " + result6 + ".";
      }
      
      // Checks if user is asking for the least called operation for 'vehicles'
      else if (least_called_operation_vehicles.includes(command_result[0][1]) == true) {
        var text = "Your least called operation for 'vehicles' is " + result7 + ".";
      }
      
      // Checks if user is asking for the least called operation for 'client-gateway-users'
      else if (least_called_operation_cgu.includes(command_result[0][1]) == true) {
        var text = "Your least called operation for 'client-gateway-users' is " + result8 + ".";
      }
      
      // Checks if user is asking for the least called operation for 'trusted-consumer'
      else if (least_called_operation_tc.includes(command_result[0][1]) == true) {
        var text = "Your least called operation for 'trusted-consumer' is " + result9 + ".";
      }
      
      // Checks if user is asking for the least called operation for 'trusted-services'
      else if (least_called_operation_ts.includes(command_result[0][1]) == true) {
        var text = "Your least called operation for 'trusted-services' is " + result10 + ".";
      }
      
      // Checks if user is asking for the 5 least called operations
      else if (least_called_operations.includes(command_result[0][1]) == true) {
        var commas = 1;
        ...
        if (commas != 5)
        {  
          text += "I'm sorry. I was not able to find 5 operations, but I was able to find the " + commas + " least called operations. ";
        };
        text += "Your least " + commas + " called operations are " + result11 + ".";
      }
      ...
      else if (most_called_operation_sessions.includes(command_result[0][1]) == true) {
        var text = "Your most called operation for 'sessions' is " + result13 + ".";
      }
      
      // Checks if user is asking for the most called operation for 'vehicles'
      else if (most_called_operation_vehicles.includes(command_result[0][1]) == true) {
        var text = "Your most called operation for 'vehicles' is " + result14 + ".";
      }
      
      // Checks if user is asking for the most called operation for 'client-gateway-users'
      else if (most_called_operation_cgu.includes(command_result[0][1]) == true) {
        var text = "Your most called operation for 'client-gateway-users' is " + result15 + ".";
      }
      
      // Checks if user is asking for the most called operation for 'trusted-consumer'
      else if (most_called_operation_tc.includes(command_result[0][1]) == true) {
        var text = "Your most called operation for 'trusted-consumer' is " + result16 + ".";
      }
      
      // Checks if user is asking for the most called operation for 'trusted-services'
      else if (most_called_operation_ts.includes(command_result[0][1]) == true) {
        var text = "Your most called operation for 'trusted-services' is " + result17 + ".";
      }
      
      // Checks if user is asking for the 5 most called operations
      else if (most_called_operations.includes(command_result[0][1]) == true) {
        var commas = 1;
        ...
        if (commas != 5)
        {  
          text += "I'm sorry. I was not able to find 5 operations, but I was able to find the " + commas + " most called operations. ";
        };
        text += "Your " + commas + " most called operations are " + result18 + ".";
      }
    ...
    /////////////////////////
      ...
      else if (avg_response_time_gv.includes(command_result[0][1]) == true) {
        var text = "The average response time for the operation 'GetVehicle' is " + result19;
      }
      
      // Checks if user is asking for the Average response time for the operation 'GetVehicle' of the API 'sessions'
      else if (avg_response_time_gv_sessions.includes(command_result[0][1]) == true) {
        var text = "The average response time for the operation 'GetVehicle' of the API 'sessions' is " + result20;
      }
      
      // Checks if user is asking for the Average response time for the operation 'GetVehicle' of the API 'client-gateway-users'
      else if (avg_response_time_gv_cgu.includes(command_result[0][1]) == true) {
        var text = "The average response time for the operation 'GetVehicle' of the API 'client-gateway-users' is " + result21;
      }
      
      // Checks if user is asking for the Average response time for the operation 'GetVehicle' of the API 'vehicles'
      else if (avg_response_time_gv_v.includes(command_result[0][1]) == true) {
        var text = "The average response time for the operation 'GetVehicle' of the API 'vehicles' is " + result22;
      }
      
      // Checks if user is asking for the Average response time for the operation 'GetAuthorizationStatus'
      else if (avg_response_time_gas.includes(command_result[0][1]) == true) {
        var text = "The average response time for the operation 'GetAuthorizationStatus' is " + result23;
      }

      // Checks if user is asking for the Average response time for the operation 'GetAuthorizationStatus' of the API 'sessions'
      else if (avg_response_time_gas_sessions.includes(command_result[0][1]) == true) {
          var text = "The average response time for the operation 'GetAuthorizationStatus' of the API 'sessions' is " + result24;
      }
      ...
      else if (avg_response_time_gas_cgu.includes(command_result[0][1]) == true) {
          var text = "The average response time for the operation 'GetAuthorizationStatus' of the API 'client-gateway-users' is " + result25;
      }

      // Checks if user is asking for the Average response time for the operation 'GetAuthorizationStatus' of the API 'vehicles'
      else if (avg_response_time_gas_v.includes(command_result[0][1]) == true) {
          var text = "The average response time for the operation 'GetAuthorizationStatus' of the API 'vehicles' is " + result26;
      }
  
      // Checks if user is asking for the Average response time for the operation 'GetUserProfile'
      else if (avg_response_time_gup.includes(command_result[0][1]) == true) {
          var text = "The average response time for the operation 'GetUserProfile' is " + result27;
      }

      // Checks if user is asking for the Average response time for the operation 'GetUserProfile' of the API 'sessions'
      else if (avg_response_time_gup_sessions.includes(command_result[0][1]) == true) {
          var text = "The average response time for the operation 'GetUserProfile' of the API 'sessions' is " + result28;
      }

      // Checks if user is asking for the Average response time for the operation 'GetUserProfile' of the API 'client-gateway-users'
      else if (avg_response_time_gup_cgu.includes(command_result[0][1]) == true) {
          var text = "The average response time for the operation 'GetUserProfile' of the API 'client-gateway-users' is " + result29;
      }

      // Checks if user is asking for the Average response time for the operation 'GetUserProfile' of the API 'vehicles'
      else if (avg_response_time_gup_v.includes(command_result[0][1]) == true) {
          var text = "The average response time for the operation 'GetUserProfile' of the API 'vehicles' is " + result30;
      }
      
      // Checks if user is asking for the Average response time for the operation 'GetSecurityQuestions'
      else if (avg_response_time_gsq.includes(command_result[0][1]) == true) {
          var text = "The average response time for the operation 'GetSecurityQuestions' is " + result31;
      }

      // Checks if user is asking for the Average response time for the operation 'GetSecurityQuestions' of the API 'sessions'
      else if (avg_response_time_gsq_sessions.includes(command_result[0][1]) == true) {
          var text = "The average response time for the operation 'GetSecurityQuestions' of the API 'sessions' is " + result32;
      }
      ...
      else if (avg_response_time_gsq_cgu.includes(command_result[0][1]) == true) {
          var text = "The average response time for the operation 'GetSecurityQuestions' of the API 'client-gateway-users' is " + result33;
      }

      // Checks if user is asking for the Average response time for the operation 'GetSecurityQuestions' of the API 'vehicles'
      else if (avg_response_time_gsq_v.includes(command_result[0][1]) == true) {
          var text = "The average response time for the operation 'GetSecurityQuestions' of the API 'vehicles' is " + result34;
      }
      ...
      // Checks if user is asking for the Average response time for the operation 'Login'
          ...
          var text = "The average response time for the operation 'Login' is " + result35;
      }

      // Checks if user is asking for the Average response time for the operation 'Login' of the API 'sessions'
      else if (avg_response_time_l_sessions.includes(command_result[0][1]) == true) {
          var text = "The average response time for the operation 'Login' of the API 'sessions' is " + result36;
      }

      // Checks if user is asking for the Average response time for the operation 'Login' of the API 'client-gateway-users'
      else if (avg_response_time_l_cgu.includes(command_result[0][1]) == true) {
          var text = "The average response time for the operation 'Login' of the API 'client-gateway-users' is " + result37;
      }

      // Checks if user is asking for the Average response time for the operation 'Login' of the API 'vehicles'
      else if (avg_response_time_l_v.includes(command_result[0][1]) == true) {
          var text = "The average response time for the operation 'Login' of the API 'vehicles' is " + result38;
      }
      ...
      {
        var text = "I'm sorry, could you be more specific? Here is a list of my commands: ";
        for (var i = 0; i < commands.length; i++) {
          text += "\n- " + bot.enrichCommand(message, commands[i]);
        }
      }
    ...
    // Prompt user to try again
        ...
        var text = "I'm sorry, I did not understand. Here is a list of my commands: ";
        for (var i = 0; i < commands.length; i++) {
          text += "\n- " + bot.enrichCommand(message, commands[i]);
        }
    }
